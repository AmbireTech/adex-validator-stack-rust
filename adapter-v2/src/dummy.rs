...
use primitives::{
    adapter::{
        ...
        Deposit, DummyAdapterOptions, Session,
    },
    config::Config,
...
pub type Adapter<S> = primitives::adapter::adapter2::Adapter<Dummy, S>;
...
impl LockedClient for Dummy {
    ...
    async fn session_from_token(&self, token: &str) -> Result<Session, Self::Error> {
        ...
        match identity {
            ...
            None => Err(Error2::authentication(format!(
...
impl UnlockedClient for Dummy {
    ...
    // requires Unlocked
    async fn get_auth(&self, _intended_for: ValidatorId) -> Result<String, Error2> {
        let who = self
            .session_tokens
            .iter()
