...
use primitives::{
    ...
    Campaign, CampaignId, UnifiedNum, ChannelId, BigNum
};
use redis::aio::MultiplexedConnection;
use deadpool_postgres::PoolError;
use slog::error;
use tokio_postgres::error::SqlState;
use crate::db::campaign::{campaign_exists, update_campaign_in_db, insert_campaign, get_campaigns_for_channel, fetch_campaign};
...
pub async fn create_campaign<A: Adapter>(
    ...
    let remaining_for_channel = get_total_remaining_for_channel(&app.redis, &app.pool, &campaign).await?;
...
async fn get_total_remaining_for_channel(redis: &MultiplexedConnection, pool: &DbPool, campaign: &Campaign) -> Result<UnifiedNum, ResponseError> {
    ...
    let latest_new_state = latest_new_state.ok_or_else(|| ResponseError::Conflict("Error getting latest new state message".to_string()))?;
...
async fn get_campaigns_remaining_sum(redis: &MultiplexedConnection, pool: &DbPool, campaign: &Campaign) -> Result<UnifiedNum, ResponseError> {
    ...
    let sum_of_campaigns_remaining = campaigns_for_channel
        ...
        .map(|c| async move {
            ...
            let remaining = c.budget.checked_sub(&spent).ok_or_else(|| {
                ResponseError::Conflict("Error while calculating remaining for campaign".to_string())
            });
            remaining
        })
        .fold(0u64, |sum: u64, val: u64| sum.checked_add(val).ok_or_else(|| {
            return ResponseError::Conflict("Error while calculating total remaining for all campaigns".to_string());
        }));
    Ok(UnifiedNum::from_u64(sum_of_campaigns_remaining))
}
...
pub async fn modify_campaign(pool: &DbPool, campaign: &Campaign, redis: &MultiplexedConnection) -> Result<bool, ResponseError> {
    ...
    let total_remaining = get_total_remaining_for_channel(&redis, &pool, &campaign).await?;
