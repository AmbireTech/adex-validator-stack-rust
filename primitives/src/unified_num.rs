...
impl UnifiedNum {
    ...
    pub fn from_precision(amount: BigNum, precision: u8) -> Option<Self> {
        ...
        let from_precision = match precision.cmp(&Self::PRECISION) {
            ...
            Ordering::Greater => amount.div_floor(&BigNum::from(10).pow(precision - Self::PRECISION)),
        };
...
mod test {
    ...
    fn test_convert_unified_num_to_new_precision_and_from_precision() {
        ...
        assert!(UnifiedNum::from_precision(larger_bignum.clone(), same_precision).is_none());
        assert_eq!(
            Some(UnifiedNum::from(184467440737095516)),
            UnifiedNum::from_precision(larger_bignum, 10),
